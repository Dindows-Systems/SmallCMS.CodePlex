    ...
    public class XmlMembershipProvider : MembershipProvider {
        ...
        public override void Initialize(string name, NameValueCollection config) {
            ...
            if (String.IsNullOrEmpty(path))
                path = Website.Properties.Settings.Default.CMS_StorageLocation + "users.xml";
        ...
        private bool CheckPassword(string storedPassword, string inputPassword)
        {
            ...
            else
            {
                ...
                {
                    if (storedPassword == Tools.HashPassword(inputPassword))
                        validated = true;
                }
            ...
            foreach (XmlNode node in nodes) 
            {
                if (node["UserName"].InnerText.Equals(username, StringComparison.OrdinalIgnoreCase)) 
                {
                    if (CheckPassword(node["Password"].InnerText, oldPassword))
                    {
                        string passwordPrep;
                        if (passwordFormat == MembershipPasswordFormat.Hashed)
                            passwordPrep = Tools.HashPassword(newPassword);
                        else
        ...
        public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status) {
            ...
            string passwordPrep;
            if (passwordFormat == MembershipPasswordFormat.Hashed)
				passwordPrep = Tools.HashPassword(password);
            else
